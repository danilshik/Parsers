...
def add_cpu_benchmark(cpu_id, id_benchmark, single_thread, multi_thread):

    sql_query = "SELECT row_id FROM cpu_benchmarks_values WHERE cpu_id = %s and cpu_benchmark_id = %s and multi_core_value = %s and single_core_value = %s LIMIT 1"
    ...
    if(len(rows) == 1):
        ...
        if(len(rows) == 0):
            sql_query = "INSERT INTO cpu_benchmarks_values (cpu_id, cpu_benchmark_id, multi_core_value, single_core_value) VALUES (%s, %s, %s, %s)"
            return insert_database(sql_query, data)
...
def parse(url):
    global count_add, count_no_add
    global count_cpu, cpus
...
def main():
    ...
    id_brenchmark_cpu = get_cpu_benchmark_id("Passmark")
...
if __name__ == '__main__':
    main()
    # list_database_product = get_product_database("cpu")
    # print(list_database_product)